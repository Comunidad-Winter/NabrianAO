...
'            frmMain.cVerBloqueos.Visible = Ver
        ...
        Case 7 'particulas
            ...
            frmMain.cQuitarParticula.Visible = Ver
            frmMain.cInsertarParticula.Visible = Ver
            frmMain.LstParticulas.Visible = Ver
        Case 8
            frmMain.cLuces.Visible = Ver
            frmMain.cInsertarLuz.Visible = Ver
            frmMain.cQuitarLuz.Visible = Ver
            frmMain.Frame2.Visible = Ver
            frmMain.RGBCOLOR.Visible = Ver
    End Select
    If Ver = True Then
        vMostrando = Numero
...
If CurrentGrh.GrhIndex = 0 Then Exit Sub
    If frmConfigSup.MOSAICO = vbUnchecked Then
        ...
        Call engine.DrawGrhtoHdc(frmMain.PreviewGrh, CurrentGrh.GrhIndex, 1, 1)
    Else
        ...
        For i = 1 To CInt(Val(frmConfigSup.mLargo))
            For j = 1 To CInt(Val(frmConfigSup.mAncho))
                ...
                SR.Right = SR.Left + GrhData(CurrentGrh.GrhIndex).pixelWidth
                SR.Bottom = SR.Top + GrhData(CurrentGrh.GrhIndex).pixelHeight
                Call engine.DrawGrhtoHdc(frmMain.PreviewGrh, CurrentGrh.GrhIndex, DR.Left, DR.Top)
