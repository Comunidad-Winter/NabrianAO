...
If deathac = False Then
 ...
 End If
        ...
        If (Death_Luchadores(i) = -1) Then
                ...
                Call SendData(ToAll, 0, 0, "||Empieza la DeathMach!!" & FONTTYPE_DEATH)
                deathesp = False
...
cantdeath = wetas
Cantidad = cantdeath
   Call SendData(ToAll, 0, 0, "||Esta empezando un nuevo deathmatch para " & cantdeath & " participantes. Para participar envia /INGRESAR. " & FONTTYPE_DEATH)
...
terminodeat = True
Call SendData(ToAll, 0, 0, "||Termina la DeathMatch! El Ganador Debe escribir /GANADOR para recibir su recompensa!" & FONTTYPE_DEATH)
End If
If Cantidad = 0 Then
   terminodeat = False
   deathesp = False
deathac = False
Call SendData(ToAll, 0, 0, "||El ganador de la deatmatch desconecto. se anulan los premios!!!" & FONTTYPE_DEATH)
End If
End If
errord:
End Sub
...
End If
    deathesp = False
    deathac = False
    Call SendData(ToAll, 0, 0, "||DeathMatch automatico cancelado por Game Master" & FONTTYPE_DEATH)
    Dim i As Integer
    For i = LBound(Death_Luchadores) To UBound(Death_Luchadores)
                If (Death_Luchadores(i) <> -1) Then
                        Dim NuevaPos As WorldPos
...
End If
    deathesp = False
    deathac = False
    Call SendData(ToAll, 0, 0, "||DeathMatch automatico cancelado por falta de participantes." & FONTTYPE_DEATH)
    Dim i As Integer
    For i = LBound(Death_Luchadores) To UBound(Death_Luchadores)
                If (Death_Luchadores(i) <> -1) Then
                        Dim NuevaPos As WorldPos
