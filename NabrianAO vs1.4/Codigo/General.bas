...
Public NoFps As Integer
Public SombrasAC As Boolean
Public RetosAC As Boolean
Public CursorGrafico As Boolean
Public DragAndDropAC As Boolean
Public CastleHechi As Boolean
...
Recompensas(HERRERO, 2, 1).Descripcion = "Permite crear las mejores armas (Espada Neithan, Espada Neithan + 1, y Daga Infernal)."
...
Recompensas(TALADOR, 1, 2).name = "Músculos Fornidos"
Recompensas(TALADOR, 1, 2).Descripcion = "Permite talar 20% más cantidad de madera."
...
Recompensas(BARDO, 3, 1).Descripcion = "Aumenta la probabilidad de Apuñalar a un 20%"
...
Recompensas(ASESINO, 3, 2).Descripcion = "Las chances de apuñalar suben a 25%."
...
EnviAdata4 = 0
        ...
        End If
    End If
ElseIf CharList(UserCharIndex).Heading <> Direction Then
If Direction = 1 Then
If EnviAdata1 = 1 Then Exit Sub
EnviAdata1 = EnviAdata1 + 1
EnviAdata2 = 0
EnviAdata3 = 0
EnviAdata4 = 0
ElseIf Direction = 2 Then
If EnviAdata2 = 1 Then Exit Sub
EnviAdata1 = 0
EnviAdata2 = EnviAdata2 + 1
EnviAdata3 = 0
EnviAdata4 = 0
ElseIf Direction = 3 Then
If EnviAdata3 = 1 Then Exit Sub
EnviAdata1 = 0
EnviAdata2 = 0
EnviAdata3 = EnviAdata3 + 1
EnviAdata4 = 0
ElseIf Direction = 4 Then
If EnviAdata4 = 1 Then Exit Sub
EnviAdata1 = 0
EnviAdata2 = 0
EnviAdata3 = 0
EnviAdata4 = EnviAdata4 + 1
End If

Call SendData("CHEA" & Direction)
End If
    ...
    Case 0
        ...
        Select Case OtherInventory(frmComerciar.List1(0).ListIndex + 1).ObjType
            Case 2
                ...
                frmComerciar.MinimoNivel.Caption = "Nivel minimo: " & OtherInventory(frmComerciar.List1(0).ListIndex + 1).MinLvl
       ...
       frmComerciar.Label1(0).Caption = PonerPuntos(UserInventory(frmComerciar.List1(1).ListIndex + 1).Valor)
        ...
        Select Case UserInventory(frmComerciar.List1(1).ListIndex + 1).ObjType
            Case 2
                ...
                frmComerciar.MinimoNivel.Caption = "Nivel minimo: " & UserInventory(frmComerciar.List1(1).ListIndex + 1).MinLvl
                frmComerciar.Label1(3).Visible = True
                frmComerciar.Label1(2).Visible = True
                frmComerciar.Label1(4).Visible = True
            Case 3
...
CastleHechi = Val(GetVar(App.Path & "/Init/Opciones.opc", "CONFIG", "Particulas"))
...
CursorGrafico = Val(GetVar(App.Path & "/Init/Opciones.opc", "CONFIG", "Cursores"))


If CursorGrafico = 0 Then 'CURSOR GRAFICO
...
lastTime = GetTickCount And &H7FFFFFFF
...
End If
    ...
    If Not EngineRun Then
        renderconnect
        timerElapsedTime = GetElapsedTime()
        timerTicksPerFrame = timerElapsedTime * 0.018
...
'While (GetTickCount - lFrameTimer) / FPslocos < FramesPerSecCounter
    ...
    'If GetTickCount - lFrameTimer >= 1100 Then
      '      FramesPerSec = FramesPerSecCounter
      '      Velocidad = 144 / FramesPerSecCounter
       ...
       ' End If
       '
        ' While (GetTickCount - lFrameTimer) \ FPslocos < FramesPerSecCounter  ' reemplazen el 18 por lo q se les cante  mientras menor sea el numero, mayor seran los FPS  es mejor q usen multiplos de 9.
        '    Sleep 5
        'Wend
    ...
    ' ### I N T E R V A L O S ###
    'esttick = GetTickCount And &H7FFFFFFF
    ...
    '
    'Next LoopC
    'ulttick = GetTickCount And &H7FFFFFFF
...
Dim grh_list() As Long
ReDim grh_list(UBound(StreamData(ParticulaInd).grh_list)) As Long
Dim i As Integer
For i = 1 To UBound(StreamData(ParticulaInd).grh_list)
grh_list(i) = StreamData(ParticulaInd).grh_list(i)
Next i
    ...
    Next map_y
        ...
        For map_x = 1 To 100
            If MapData(map_x, map_y).Graphic(2).GrhIndex > 0 Then
                SetPixel frmPrincipal.Minimap.hDC, map_x, map_y, GrhData(MapData(map_x, map_y).Graphic(2).GrhIndex).MiniMap_color
            End If
        Next map_x
    Next map_y
        ...
        For map_x = 1 To 100
            If MapData(map_x, map_y).Graphic(3).GrhIndex > 0 Then
                SetPixel frmPrincipal.Minimap.hDC, map_x, map_y, GrhData(MapData(map_x, map_y).Graphic(3).GrhIndex).MiniMap_color
            End If
        Next map_x
    Next map_y
    ...
    If Not bTecho Then
        ...
        For map_x = 1 To 100
            If MapData(map_x, map_y).Graphic(4).GrhIndex > 0 Then
                SetPixel frmPrincipal.Minimap.hDC, map_x, map_y, GrhData(MapData(map_x, map_y).Graphic(4).GrhIndex).MiniMap_color
            End If
        Next map_x
    Next map_y
    Else
